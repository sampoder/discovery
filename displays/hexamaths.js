...
function generateTheQuestion() {
  if (Math.random() > 0.5) {
    ...
    document.getElementById(`a-${answerLocation}`).innerText = decimalToHexString(result)
    document.getElementById(`a-${answerLocation}`).onclick = function () { alert(`🎉 Hooray! Well done, that's right!`); generateTheQuestion(); };
    for (let x = 0; x < 4; x++) {
      if (x != answerLocation) {
        document.getElementById(`a-${x}`).innerText = decimalToHexString(randomIntFromInterval(0, 255))
        document.getElementById(`a-${x}`).onclick = function () { alert(`😭 Oh no! Sadly, that's wrong :(`); generateTheQuestion(); };
      }
    }
  }
  else {
    ...
    if (Math.random() > 0.5) {
      if (firstNumber > secondNumber) {
        result = firstNumber - secondNumber
        string = `${decimalToHexString(firstNumber)} - ${decimalToHexString(secondNumber)} = ?`
      }
      else {
        result = secondNumber - firstNumber
        string = `${decimalToHexString(secondNumber)} - ${decimalToHexString(firstNumber)} = ?`
      }
    }
    else {
      result = firstNumber + secondNumber
    ...
    for (let x = 0; x < 4; x++) {
      if (x != answerLocation) {
        document.getElementById(`a-${x}`).innerText = randomIntFromInterval(0, 255)
        document.getElementById(`a-${x}`).onclick = function () { alert(`😭 Oh no! Sadly, that's wrong :(`); generateTheQuestion(); };
      }
    }
  }
