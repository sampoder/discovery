...
function calculateSine(theXCord, currentFrameCount) {
  return (Math.sin((degreesToRadians(((internalMultiplier) * (theXCord + currentFrameCount)) + internalAddition))) * externalMultiplier) + externalAddition
}
...
function setup() {
  ...
  background(255);
  noStroke()
  textFont(`system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Roboto', 'Oxygen', 'Ubuntu', 'Cantarell', 'Fira Sans', 'Droid Sans', 'Helvetica Neue', 'Segoe UI Emoji', 'Apple Color Emoji', 'Noto Color Emoji', sans-serif`);
  textStyle(BOLD);
}
...
function draw() {
  ...
  for (let i = 30; i < (width - windowHeight - 50); i++) {
    ...
    vertex(theXCord, presentSine(calculateSine(theXCord, frameCount)));
    endShape();
    document.getElementById('preview').style.background = `rgb(0, 0, ${Math.abs(calculateSine(theXCord, frameCount))})`
    document.getElementById('preview').innerHTML = `<div>${Math.round(Math.abs(calculateSine(theXCord, frameCount)))}</div>`
  }
  fill(225)
  noStroke()
